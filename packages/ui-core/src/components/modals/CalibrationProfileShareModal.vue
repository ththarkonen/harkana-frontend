<template>
<Modal ref = "modal"
	   title = "Calibration profile sharing"
	...
	<template #main>
		...
		<div v-else class = "space-y-4">
			...
			<div class = "space-y-2">
				<label for = "calibration-profile-share-email"
					   class = "block text-xs font-semibold uppercase tracking-wide text-white/70">
					Add collaborator
				</label>
				<input id = "calibration-profile-share-email"
					   v-model = "email"
					   type = "email"
...
import { data as datalib } from "@harkana/tools"
...
const props = defineProps({
	...
	axisRole: { type: String, default: "spectrum-x" }
})
...
const refreshCollaborators = async () => {
	...
	const response = await datalib.listCalibrationProfileCollaborators(
...
const addCollaborator = async () => {
	...
	try{
		...
		await datalib.addCalibrationProfileCollaborator(
			activeProfileID.value,
			normalizedEmail,
			props.dataType,
			props.axisRole
		...
		errorMessage.value = String( error?.detail ?? error?.message ?? "Failed to share calibration profile." ).trim()
		console.log( error )
	} finally {
		sharing.value = false
	}
...
const removeCollaborator = async ( collaborator ) => {
	...
	try{
		errorMessage.value = ""
		sharing.value = true
		await datalib.removeCalibrationProfileCollaborator(
			activeProfileID.value,
			collaboratorID,
			props.dataType,
			props.axisRole
